...
class database {
					...
					while (true) {
						try {
							...
							if (isBusyError && attempt < this.busyRetryCount) {
								attempt++;
								console.warn(
									`Database busy, retrying operation (attempt ${attempt}/${this.busyRetryCount})...`
								);
								// Wait before retrying with exponential backoff
								await new Promise((r) =>
									setTimeout(r, this.busyRetryDelay * attempt)
								);
							} else {
								reject(error);
								throw error;
		...
		return this.execute(async () => {
			...
			if (tableName === "accounts" && key !== undefined) {
				...
				if (!Array.isArray(allAccounts) || allAccounts.length === 0) {
					console.warn(
						`Account with ID ${keyId} not found - no accounts available`
					);
					return undefined;
				}
				...
				let account = allAccounts.find(
					(acc) => acc && Number(acc.ID) === keyId
				);

				// If not found with number comparison, try with string comparison
				if (!account) {
					account = allAccounts.find(
						(acc) => acc && String(acc.ID) === String(keyId)
					);

					if (account) {
						console.log(
							`Found account: ${account.name} with ID: ${account.ID} using string comparison`
						);
					}
				} else {
					console.log(
						`Found account: ${account.name} with ID: ${account.ID} using number comparison`
					);
				}
		...
		return this.execute(async () => {
			if (tableName === "accounts") {
				...
				console.log(
					...
					} accounts with IDs: ${normalizedAccounts
						.map((a) => a?.ID)
						.join(", ")}`
				);
	...
	async updateData(tableName, data, key = 1) {
		return this.execute(async () => {
			...
			if (tableName === "accounts" && key !== 1) {
				...
				if (!accountExists) {
					console.log(
						`Account with ID ${key} does not exist, attempting to add instead of update`
					);
				...
				if (!Array.isArray(dataCopy)) {
					console.warn(
						"[updateData] Received an object instead of an array for accounts"
					);
					...
					if (
						...
						dataCopy.ID !== undefined &&
						dataCopy.name
					) {
						console.log(
							"[updateData] Converting individual account to array for storage"
						);
						...
						const accountExists =
							...
							allAccounts.some(
								(acc) =>
									Number(acc.ID) === Number(dataCopy.ID) ||
						...
						if (accountExists) {
							...
							dataCopy = Array.isArray(allAccounts)
								? [...allAccounts, dataCopy]
								: [dataCopy];
							console.log(`Added account ${dataCopy.ID} to accounts array`);
						}
					} else {
						console.error(
							"[updateData] Invalid data received to update accounts"
						);
						// Get current accounts as backup
						const currentAccounts =
							(await encryptedStorage.loadData("accounts")) || [];

						if (Array.isArray(currentAccounts) && currentAccounts.length > 0) {
							console.warn(
								"[updateData] Using existing accounts instead of invalid data"
							);
							dataCopy = currentAccounts;
						} else {
							console.warn(
								"[updateData] No existing accounts, using empty array"
							);
							dataCopy = [];
						}
					}
				...
				console.log(
					`[updateData] Saving array of ${
						Array.isArray(dataCopy) ? dataCopy.length : 0
					} accounts`
				);
				if (Array.isArray(dataCopy) && dataCopy.length > 0) {
					console.log(
						`Saving accounts with IDs: ${dataCopy
							.map((acc) => acc?.ID)
							.join(", ")}`
					);
				}
	...
	async deleteData(tableName, key = 1) {
		return this.execute(async () => {
			...
			if (tableName === "accounts") {
				...
				const accountWithId1Exists =
					Array.isArray(allAccounts) &&
					allAccounts.some(
						(acc) => acc && (Number(acc.ID) === 1 || String(acc.ID) === "1")
					);

				// Si key === 1 pero no existe una cuenta con ID 1, bloqueamos la operación
				if (key === 1 && !accountWithId1Exists) {
					console.error(
						"PROTECCIÓN CRÍTICA: Se intentó eliminar todas las cuentas. Operación bloqueada."
					);
					return false;
				}
				...
				const accountToDelete = allAccounts.find(
					(acc) =>
						acc && (Number(acc.ID) === key || String(acc.ID) === String(key))
				);
				...
				console.log(
					`Found account for deletion: ${accountToDelete.name} with ID ${accountToDelete.ID}`
				);
				...
				try {
					...
					if (fs.existsSync(accountsPath)) {
						await encryptedStorage.createSingleBackup(
							accountsPath,
							"pre-delete-backup"
						);
					}
				...
				if (stillExists) {
					console.error(
						`Account with ID ${key} still exists after deletion attempt!`
					);
					return false;
				}
				...
				if (remainingAccounts.length === 0) {
					console.warn(
						"La última cuenta fue eliminada. El archivo de cuentas está ahora vacío."
					);
				}

				console.log(
					`Successfully deleted account with ID: ${key}, remaining accounts: ${remainingAccounts.length}`
				);
				return deleted;
			} else {
			...
			throw new Error(
				`Could not delete data in ${tableName}: ${error.message}`
			);
		});
	}
	...
	async getSelectedAccount() {
		try {
			...
			const account = accounts.find(
				(acc) =>
					acc &&
					(Number(acc.ID) === Number(selectedId) ||
			...
			console.log(
				`Found selected account: ${account.name} with ID: ${account.ID}`
			);
		...
		try {
			...
			const fixedAccounts = accounts
				.map((acc, index) => {
					if (!acc) {
						console.warn(
							`Found null/undefined account at index ${index}, skipping`
						);
						needsFix = true;
					...
					if (acc.ID === undefined) {
						...
						console.warn(
							`Account ${
								acc.name || "unknown"
							} is missing ID, assigning new one`
						);
						needsFix = true;
						const newId = index + 1;
						return { ...acc, ID: newId };
					...
					if (isNaN(id)) {
						...
						console.warn(
							`Account ${acc.name || "unknown"} has invalid ID (${
								acc.ID
							}), assigning new one`
						);
						needsFix = true;
					...
					if (seenIds.has(id)) {
						...
						console.warn(
							`Found duplicate ID ${id} for account ${acc.name}, reassigning to ${newId}`
						);
						seenIds.add(newId);
						return { ...acc, ID: newId };
					}
			...
			if (needsFix) {
				...
				console.log(
					`Saving accounts with IDs: ${fixedAccounts
						.map((acc) => acc?.ID)
						.join(", ")}`
				);
				...
				console.log(
					`Verified accounts after fix: ${verifiedAccounts
						.map((acc) => acc?.ID)
						.join(", ")}`
				);
			}
	...
	async setSelectedAccount(accountId) {
		try {
			...
			if (!account) {
				console.warn(
					`Cannot select non-existent account with ID: ${accountId}`
				);
				return false;
			}
			...
			if (!config || typeof config !== "object") {
				console.warn(
					"configClient no existe, no se puede seleccionar la cuenta"
				);
				return false;
			}
	...
	/**
	 * Migrate account data to new authentication format
	 * @returns {Promise<boolean>} - true if migration was successful
	 */
	async migrateAccountsToNewAuth() {
		try {
			console.log(
				"Checking if account migration to new auth system is needed..."
			);
			const accounts = await this.readAllData("accounts");
			...
			const migratedAccounts = accounts.map((account) => {
				...
				// Check if this is an old Microsoft/Xbox account that needs migration
				if (
					account.meta &&
					(account.meta.type === "Xbox" || account.meta.type === "Microsoft")
				) {
					// If it doesn't have a refresh_token but has old-style tokens, mark for migration
					if (
						!account.refresh_token &&
						(account.access_token || account.mcToken)
					) {
						console.log(
							`Account ${account.name} needs migration to new auth format`
						);
						needsMigration = true;
			...
			if (needsMigration) {
				console.log("Migrating accounts to new authentication format...");
				await this.updateData("accounts", migratedAccounts);
